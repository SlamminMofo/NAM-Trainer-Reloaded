...
_PLAYER_COMPATIBLE_MODEL_VERSION = "0.5.4"
...
class LSTM(_BaseNet, _ImportsWeights):
    ...
    def _get_export_dict(self):
        ...
        model_dict["version"] = _PLAYER_COMPATIBLE_MODEL_VERSION
        return model_dict
...
class CausalConvLSTM(_BaseNet, _ImportsWeights):
    """
    Dense(tanh) -> causal Conv1D -> LSTM -> Dense -> causal Conv1D.

    This custom recurrent topology exports as CausalConvLSTM for NAM Player
    Extended builds. Standard NAM Player builds do not support it.
    """
    ...
    def __init__(
        ...
        num_layers: int = 1,
        input_size: int = 1,
        input_conv_kernel_size: int = 128,
        output_conv_kernel_size: int = 2048,
        train_burn_in: _Optional[int] = None,
        train_truncate: _Optional[int] = None,
        sample_rate: _Optional[float] = None,
        **lstm_kwargs,
        ...
        if "batch_first" in lstm_kwargs:
            raise ValueError("batch_first cannot be set.")
        if int(input_size) != 1:
            raise ValueError("CausalConvLSTM only supports input_size=1.")

        self._input_size = int(input_size)
        self._input_conv_kernel_size = int(input_conv_kernel_size)
        self._output_conv_kernel_size = int(output_conv_kernel_size)
        self._input_dense = _nn.Linear(1, 1, bias=False)
        self._input_conv = _nn.Conv1d(
            1,
            1,
            kernel_size=self._input_conv_kernel_size,
            bias=False,
        )
        self._core = _L(
            self._input_size,
        ...
        self._head = self._init_head(hidden_size)
        self._output_conv = _nn.Conv1d(
            1,
            1,
            kernel_size=self._output_conv_kernel_size,
            bias=False,
        )
        self._train_burn_in = train_burn_in
        self._train_truncate = train_truncate
        self._initial_cell = _nn.Parameter(_torch.zeros((int(num_layers), hidden_size)))
        self._initial_hidden = _nn.Parameter(_torch.zeros((int(num_layers), hidden_size)))
        self._get_initial_state_burn_in = 48_000
        self._initialize_tonex_like_layers()

    def _initialize_tonex_like_layers(self):
        _nn.init.xavier_uniform_(self._input_dense.weight)
        _nn.init.xavier_uniform_(self._head.weight)
        _nn.init.zeros_(self._head.bias)
        with _torch.no_grad():
            self._input_conv.weight.zero_()
            self._input_conv.weight[0, 0, -1] = 1.0
            self._output_conv.weight.zero_()
            self._output_conv.weight[0, 0, -1] = 1.0
    ...
    def import_weights(self, weights):
        ...
        def assign_tensor(tensor: _torch.Tensor, i_weight: int) -> int:
            values = weights[i_weight : i_weight + tensor.numel()]
            if len(values) != tensor.numel():
                raise ValueError(
                    f"Weight list ended while assigning tensor with numel {tensor.numel()}."
                )
            tensor.data = _torch.from_numpy(values).reshape(tensor.shape).to(tensor.device)
            return i_weight + tensor.numel()

        def import_lstm_cell_weights(cell_index: int, i_weight: int) -> int:
            def assign_state(state: _nn.Parameter, i_weight: int) -> int:
                values = weights[i_weight : i_weight + self._core.hidden_size]
                if len(values) != self._core.hidden_size:
                    raise ValueError(
                        f"Weight list ended while assigning LSTM layer {cell_index} state."
                    )
                state.data[cell_index] = _torch.from_numpy(values).to(state.device)
                return i_weight + self._core.hidden_size
            ...
            w = _torch.from_numpy(weights[i_weight : i_weight + nw]).reshape(w_shape)
            w = w.to(self.input_device)
            if w.numel() != nw:
                raise ValueError(
                    f"Weight list ended while assigning LSTM layer {cell_index}."
                )
            getattr(self._core, f"weight_ih_l{cell_index}").data = w[:, :input_dim]
            getattr(self._core, f"weight_hh_l{cell_index}").data = w[:, input_dim:]
            i_weight += nw
            i_weight = assign_tensor(getattr(self._core, f"bias_ih_l{cell_index}"), i_weight)
            getattr(self._core, f"bias_hh_l{cell_index}").data.zero_()
            i_weight = assign_state(self._initial_hidden, i_weight)
            i_weight = assign_state(self._initial_cell, i_weight)
            return i_weight

        weight_index = assign_tensor(self._input_dense.weight, weight_index)
        weight_index = assign_tensor(self._input_conv.weight, weight_index)
        for layer_index in range(self._core.num_layers):
            weight_index = import_lstm_cell_weights(layer_index, weight_index)
        head_weight_count = self._head.weight.numel() + self._head.bias.numel()
        self._head.import_weights(weights[weight_index : weight_index + head_weight_count])
        weight_index += head_weight_count
        weight_index = assign_tensor(self._output_conv.weight, weight_index)
        if weight_index != len(weights):
            raise ValueError(
                f"Unexpected extra CausalConvLSTM weights: {len(weights) - weight_index}."
            )

    def _causal_conv(self, conv: _nn.Conv1d, x: _torch.Tensor) -> _torch.Tensor:
        x = x.transpose(1, 2)
        x = _F.pad(x, (conv.kernel_size[0] - 1, 0))
    ...
    def _forward(
        self,
        x: _torch.Tensor,
        initial_state: _Optional[_LSTMHiddenCellType] = None,
    ) -> _torch.Tensor:
        def process_in_blocks(x, hidden_state=None):
            block_size = 65_535
            outputs = []
        ...
        x = _torch.tanh(self._input_dense(x))
        x = self._causal_conv(self._input_conv, x)
        ...
        x = self._apply_head(output_features)
        return self._causal_conv(self._output_conv, x)[:, :, 0]
    ...
    def _export_config(self):
        return {
            ...
            "hidden_size": self._core.hidden_size,
            "num_layers": self._core.num_layers,
            "input_conv_kernel_size": self._input_conv_kernel_size,
            "output_conv_kernel_size": self._output_conv_kernel_size,
        }

    def _get_export_dict(self):
        model_dict = super()._get_export_dict()
        model_dict["architecture"] = "CausalConvLSTM"
        model_dict["version"] = _PLAYER_COMPATIBLE_MODEL_VERSION
        return model_dict
    ...
    def _export_weights(self):
        return _np.concatenate(
            [
                self._input_dense.weight.data.detach().cpu().numpy().flatten(),
                self._input_conv.weight.data.detach().cpu().numpy().flatten(),
            ]
            + [
                self._export_cell_weights(i, h, c)
                for i, (h, c) in enumerate(zip(*self._get_initial_state()))
            ...
            + [
                self._head.export_weights(),
                self._output_conv.weight.data.detach().cpu().numpy().flatten(),
            ]
        )

    def _get_initial_state(self, inputs=None) -> _LSTMHiddenCellType:
        inputs = (
            _torch.zeros((1, self._get_initial_state_burn_in, self._input_size))
            if inputs is None
            else inputs
        ).to(self.input_device)
        _, (h, c) = self._core(inputs)
