...
from .recurrent import CausalConvLSTM as _CausalConvLSTM
...
def _init_causal_conv_lstm(
    config, sample_rate: _Optional[float]
) -> _CausalConvLSTM:
    return _CausalConvLSTM(sample_rate=sample_rate, **config)
...
def _init_slimmable_container(config, sample_rate: _Optional[float]) -> _BaseNet:
    ...
    submodel_dict = _deepcopy(highest_quality["model"])
    if sample_rate is not None and "sample_rate" not in submodel_dict:
        submodel_dict["sample_rate"] = sample_rate
    return init_from_nam(submodel_dict)
